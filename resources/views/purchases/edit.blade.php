...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('purchases.show', $purchase) }}" class="w-10 h-10 rounded-full bg-white/60 border border-white/40 flex items-center justify-center active:scale-95 transition-transform">
                <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <h2 class="text-lg font-bold text-dark">Edit Nota Pembelian</h2>
        </div>
    </x-slot>
                                        ...
                                        <select :name="'items['+index+'][product_id]'" 
                                                x-init="
                                                    $nextTick(() => {
                                                        ...
                                                        if (item.productId) {
                                                            $($el).val(item.productId).trigger('change.select2');
                                                        }
                                                    });
                                                "
                                                required
            ...
            <div class="lg:col-span-1 space-y-5">
                ...
                <div class="card-solid p-5 space-y-4">
                            ...
                            <div class="flex-1">
                                <select name="supplier_id" id="supplier-select"
                                        x-init="
                                            ...
                                            $($el).val(supplierId).trigger('change.select2');
                                        "
                                        required
                    ...
                    <div class="space-y-3 pt-2 border-t border-gray-150">
                        <div>
                            ...
                            <input type="text" name="supplier_invoice_number" x-model="supplierInvoiceNumber" placeholder="Masukkan nomor nota dari tengkulak..."
                                   class="form-input-solid">
                        </div>
                        <div>
                ...
                <div class="card-solid p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-2 border-b border-gray-100/50 pb-3">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 mb-1">Biaya Tambahan (Rp)</label>
                        ...
                        <div>
                            ...
                            <input type="text" name="additional_cost_notes" x-model="additionalCostNotes"
                                   placeholder="Contoh: Parkir & Bensin" class="form-input-solid">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" x-model="notes" rows="2" placeholder="Catatan transaksi..." class="form-input-solid"></textarea>
                    </div>

                    <button type="submit" class="btn-primary w-full py-3.5 font-bold rounded-xl shadow-lg transition-transform active:scale-[0.98]">Simpan Perubahan</button>
                </div>
            </div>
            ...
            return {
                ...
                items: [
                    @foreach($purchase->purchaseItems as $item)
                    {
                        ...
                        buyUnit: '{{ $item->product->buyUnit->symbol }}',
                        priceHistory: []
                    },
                    @endforeach
                ],
                ...
                additionalCost: {{ $purchase->additional_cost ?? 0 }},
                additionalCostNotes: {!! json_encode($purchase->additional_cost_notes) !!},
                notes: {!! json_encode($purchase->notes) !!},
                supplierInvoiceNumber: {!! json_encode($purchase->supplier_invoice_number) !!},
                ...
                // Gallery State
                imagePreview: '{{ $purchase->invoice_image ? asset('storage/' . $purchase->invoice_image) : null }}',
                dragging: false,
                galleryFilepath: '{{ $purchase->invoice_image }}',
                ...
                init() {
                    // Set up initial Select2 values and price histories
                    this.$nextTick(() => {
                        this.items.forEach((item, index) => {
                            const selectEl = $(`.form-input-solid[name="items[${index}][product_id]"]`);
                            if (selectEl.length && item.productId) {
                                selectEl.val(item.productId).trigger('change.select2');
                            }
                        });
                        this.fetchPriceHistoryForAll();
                    });
