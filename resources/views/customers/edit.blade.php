<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('customers.show', $customer) }}" class="w-10 h-10 rounded-full bg-white/60 border border-white/40 flex items-center justify-center active:scale-95 transition-transform hover:bg-white shadow-sm">
                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </a>
                <div>
                    <h2 class="text-lg font-bold text-dark">Edit Data Pelanggan</h2>
                    <p class="text-xs text-gray-400 hidden sm:block">Perbarui informasi kontak atau data petani {{ $customer->name }}</p>
                </div>
            </div>
            <div class="hidden sm:flex items-center gap-2">
                <a href="{{ route('customers.show', $customer) }}" class="px-4 py-2 text-xs font-semibold text-gray-600 bg-white hover:bg-gray-50 rounded-xl border border-gray-200 transition-colors shadow-2xs">
                    Batal
                </a>
                <button type="submit" form="customer-edit-form" class="btn-primary py-2 px-4 text-xs font-bold rounded-xl shadow-xs active:scale-95 transition-transform flex items-center gap-1.5">
    ...
    <div class="py-5 pb-24 max-w-2xl mx-auto">
        <div class="card-solid p-5 sm:p-6">
            <form id="customer-edit-form" action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="name" class="block text-xs font-semibold text-gray-500 mb-1">Nama Pelanggan / Petani <span class="text-red-500">*</span></label>
                        <div class="input-group-solid">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $customer->name) }}" required placeholder="Nama petani/pelanggan..."
                ...
                <div>
                    ...
                    <div class="input-group-solid">
                        ...
                        <textarea name="address" id="address" rows="3" placeholder="Alamat lengkap..."
                                  class="form-input-solid">{{ old('address', $customer->address) }}</textarea>
                    </div>
                    @error('address')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="pt-2 flex items-center justify-end gap-3 border-t border-gray-150">
                    <a href="{{ route('customers.show', $customer) }}" class="px-5 py-2.5 text-xs font-semibold text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors">
                        Batal
                    </a>
